...
export function ProximalCrossSection(props: Props) {
  ...
  const extent = Math.max(1, ...sections.map(s => Math.abs(s.x) + Math.max(s.outer, s.inner))) * 1.2;
  ...
  return (
    <section aria-label="近位端の断面図" className="w-64 shrink-0 rounded-lg border border-gray-700 bg-gray-900/70 px-3 py-2">
      ...
      {sections.length ? <>
        <svg role="img" aria-label="選択デバイスの近位径の組み合わせ" viewBox={`${-extent} ${-extent} ${extent * 2} ${extent * 2}`} className="mx-auto h-36 w-full">
          {sections.map(s => (
            <g key={s.label}>
              <title>{s.label}: {s.device.name}</title>
              ...
              <text x={s.x} y={-s.outer - extent * 0.035} fill={s.color} fontSize={extent * 0.13} textAnchor="middle">{s.label}</text>
            </g>
          ))}
        </svg>
        <p className="text-[10px] leading-4 text-gray-400">G：ガイディング／①②：内腔／a b：マイクロ</p>
