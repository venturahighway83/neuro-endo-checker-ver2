...
export function routeCatheters(tubes: readonly RoutedSpec[], exposure: number): Map<string, RoutedTube> {
  ...
  function route(tube: RoutedSpec): RoutedTube {
    ...
    if (!parent) {
      ...
      const bend = new THREE.CubicBezierCurve3(start, start.clone().addScaledVector(inward, joinDistance * 0.6), join.clone().addScaledVector(tangent, -joinDistance * 0.6), join);
    ...
    const result = { path, rootRotation: new THREE.Quaternion().setFromUnitVectors(zAxis, path.getTangentAt(0)), port, inlet };
