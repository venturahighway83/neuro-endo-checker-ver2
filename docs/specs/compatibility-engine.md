...
外側デバイスを選択すると、内側の候補から個別判定が `incompatible` のデバイスを非表示にする。
同じ外側にもう1本選択されている場合は、2本同時挿入が `incompatible` となる候補も非表示にする。
これは①・②のどちらを変更する場合も適用し、中間内のマイクロにもそれぞれの親に対して適用する。
`unknown` と `warning` は候補に残す。外側未選択の場合は全候補を表示する。
外側・もう1本の選択変更に追従して候補を再計算する。すでに選択済みのデバイスは自動解除せず、不適合なら既存の判定欄で表示する。
